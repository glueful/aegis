...
namespace Glueful\Extensions\Aegis\Tests\Unit\Schema;
...
final class SchemaManifestTest extends TestCase
{
    ...
    public function testVerifierClassConformsToTheManifestContract(): void
    {
        ...
        self::assertSame('glueful/aegis', (new $class())->source());
    }
    ...
    public function testVerifierCoversEveryRecursivelyDiscoveredMigration(): void
    {
        $class = $this->manifest()['migrations'][0]['verifier'];
        $verifier = new $class();
        self::assertInstanceOf(\Glueful\Extensions\Aegis\Schema\AegisSchemaVerifier::class, $verifier);
        $mapped = $verifier->migrationBasenames();
    ...
    {
        // The empty requires.extensions pin: aegis must resolve with no other extension enabled.
        // The version under test is the declared runtime floor itself — Version::VERSION on the
        // local checkout lags until the 1.79.0 release sitting.
        $g = $this->manifest();
        $candidates = ['glueful/aegis' => new \Glueful\Extensions\ExtensionCandidate(
            name: 'glueful/aegis',
            provider: $g['provider'],
        ...
        self::assertSame([], $result->errors, 'aegis must resolve enabled-alone');
    }
